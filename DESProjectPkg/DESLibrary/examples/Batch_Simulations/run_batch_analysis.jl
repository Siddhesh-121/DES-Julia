...
println("🏗️  MMC Batch Analysis Framework")
println("=" ^ 60)
...
function analyze_mmc_results(mmc_result)
    if mmc_result === nothing
        println("⚠️ Cannot perform analysis - MMC simulation failed")
        return nothing
    end
    ...
    @printf("Runtime consistency:  %s (CV: %.4f)\n", 
            mmc_consistent ? "✅ High" : "⚠️ Moderate", mmc_stats.runtime_stats.cv)
