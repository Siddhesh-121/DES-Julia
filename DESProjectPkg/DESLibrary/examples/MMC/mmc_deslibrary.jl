...
println("🏗️  M/M/C Queue Simulation using DESLibrary")
println("=" ^ 50)
...
if length(server_utils) > 1
    ...
    println("  Balance status: $(server_cv < 0.01 ? "✅ Excellent" : server_cv < 0.05 ? "✅ Good" : "⚠️ Fair")")
end
...
if ρ_per_server >= 1.0
    ...
    if c == 1
        ...
        global sum_term = 0.0
        for k in 0:(c-1)
            global sum_term += (ρ^k) / factorial(k)
        end
